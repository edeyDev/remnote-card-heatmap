...
export const CardHeatmap = () => {
  ...
  const rows = useTracker(async () => {
    ...
    for (const rem of rems) {
      ...
      for (const card of cards ?? []) {
        const stats = classify(card.repetitionHistory, card.timesWrongInRow ?? 0);
        result.push({
          id: card._id,
          remId: rem._id,
  ...
  return (
    <section className="ch-card" aria-label="Heatmap de dificultad de tarjetas">
      ...
      <div className="ch-note">Haz clic en una tarjeta para abrir su Rem. La clasificación usa el historial de respuestas: rojo = dificultad alta, verde = dominio.</div>
    </section>
  );
};
